...
export class ModuloComponent implements AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.firestore
      ...
      .valueChanges()
      .subscribe((mecanicos) => {
        this.totalMecanicosActivos = mecanicos.filter(
          (mecanico) => mecanico.isAviable === true
        ).length;
      });
    ...
    this.firestore
      ...
      .valueChanges()
      .subscribe((users) => {
        this.totalUsuariossActivos = users.filter(
          (users) => users.isAviable === true
        ).length;
      });

    this.firestore
      .collection<Membresia>('membresias')
      .valueChanges()
    ...
    this.firestore
      .collection<MecanicoData>('mecanicos')
      .valueChanges()
      .subscribe((mecanicos) => {
        const generosMecanicos = mecanicos.map((mecanico) => mecanico.gender);
        const hombresMecanicos = generosMecanicos.filter(
          (genero) => genero === 'Masculino'
        ).length;
        const mujeresMecanicos = generosMecanicos.filter(
    ...
    this.firestore
      .collection<ClienteData>('users')
      .valueChanges()
      .subscribe((users) => {
        const generosUsuarios = users.map((user) => user.gender);
        const hombresClientes = generosUsuarios.filter(
          (genero) => genero === 'Masculino'
        ).length;
        const mujeresClientes = generosUsuarios.filter(
          (genero) => genero === 'Femenino'
        ).length;
    ...
    this.firestore
      .collection<ServicioData>('services')
      .valueChanges()
      .subscribe((service) => {
        const totalServicios = service.map(
          (service) => service.customerAcceptance
        );
        const totalOk = totalServicios.filter(
          (servicio) => servicio === 'Ok'
        ).length;
        const pending = totalServicios.filter(
          (servicio) => servicio === 'pending'
        ).length;
        const totalNoOk = totalServicios.filter(
          (servicio) => servicio === 'NoOk'
        ).length;
        this.createBarChartServices(totalOk, totalNoOk, pending);
      });

    this.firestore
      .collection<RequestData>('requests')
      .valueChanges()
      .subscribe((soli) => {
        const totalSolicitudes = soli.map((soli) => soli.status);
        const totlaPending = totalSolicitudes.filter(
          (solicitud) => solicitud === 'pending'
        ).length;
        const totalInProcess = totalSolicitudes.filter(
          (solicitud) => solicitud === 'inProcess'
        ).length;
        const totalBilling = totalSolicitudes.filter(
          (solicitud) => solicitud === 'inBilling'
        ).length;
        const totalCustomer = totalSolicitudes.filter(
          (solicitud) => solicitud === 'inCustomerAcceptance'
        ).length;
        const totalSelecting = totalSolicitudes.filter(
          (solicitud) => solicitud === 'selecting'
        ).length;
        const totalFinished = totalSolicitudes.filter(
          (solicitud) => solicitud === 'finished'
        ).length;
        const totalRejected = totalSolicitudes.filter(
          (solicitud) => solicitud === 'inBilling'
        ).length;
        this.createColumnChartSolicitudes(
          totlaPending,
          totalInProcess,
          totalBilling,
          totalCustomer,
    ...
    combineLatest([
      this.firestore
        .collection<MechanicQualification>('mechanicQualifications')
        .valueChanges(),
      ...
      qualifications.forEach((qualification) => {
        ...
        mechanicRatingsMap[qualification.mechanicId].push(
          qualification.qualification
        );
      });
      this.mechanicRatings = Object.keys(mechanicRatingsMap).map(
        (mechanicId) => {
  ...
  createProfitsChart() {
    ...
    this.firestore
      .collection<Membresias>('membresias')
      .valueChanges()
      .subscribe((data) => {
  ...
  fechaUsuarios() {
    interface ClientesFecha {
      name: string;
      createdAt: Date | string;
    }
    this.firestore
      .collection<ClientesFecha>('users')
      .valueChanges()
      .subscribe((data) => {
        const chartDataM = data.map((item) => {
          console.log('Fecha:', item.createdAt, 'Nombre:', item.name);
          const date = new Date(item.createdAt);
          return {
            x: Date.UTC(date.getFullYear(), date.getMonth(), 1),
            y: 1,
            name: item.name,
          };
        });
        ...
        Highcharts.chart('fechaUsuarios', chartOptionsM);
      });
  }

  fechaMecanicos() {
    interface MecanicosFecha {
      name: string;
      createdAt: Date | string;
    }
    this.firestore
      .collection<MecanicosFecha>('mecanicos')
      .valueChanges()
      .subscribe((data) => {
        const chartDataM = data.map((item) => {
          console.log('Fecha:', item.createdAt, 'Nombre:', item.name);
          const date = new Date(item.createdAt);
          return {
            x: Date.UTC(date.getFullYear(), date.getMonth(), 1), // Utiliza el primer día del mes como valor X
            y: 1,
            name: item.name,
          };
        });
        ...
        const chartOptionsM: Highcharts.Options = {
          ...
          xAxis: {
            type: 'datetime', // Configura el eje X como "datetime"
            labels: {
              formatter: function () {
                return Highcharts.dateFormat('%b %Y', Number(this.value)); // Convierte el valor a número
              },
            },
            title: {
              text: 'Mecanicos',
        ...
        Highcharts.chart('fechaMecanicos', chartOptionsM);
      });
  }
